...
calc_AQI <- function(df,poll.list,suffix=NULL,keep_all=F){
  ref_index <- read_excel("/var/rprojects/EEA/EEA_AQIndex/data/indexEU.xlsx")
...
#With decimals
calc_AQI_dec <- function(df,poll.list,keep.all=F,suffix=NULL){
  ref_index <- read_excel("/var/rprojects/EEA/EEA_AQIndex/data/indexEU.xlsx")
  ref_index <- filter(ref_index, country == 'EU')
  ...
  if(keep.all){
    indexes <- df %>% select(-int_index,-r1,-r2,-add_decimals)
  }
