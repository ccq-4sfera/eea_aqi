...
#NOTE that some observations are missing because they lie outside cams boundaries
...
N = 1  #number of (worst) stations to compare
# ---
...
for(station in station.list){
  ...
  if(exists("obs_cams")){
    ...
    if(get_forecast){
      ##.Get CAMS Forecast+Analysis from api ----
      # get station latlon (needed to get Forecast+analysis)
      latlon <- dtable %>% dplyr::filter(AirQualityStationEoICode %in% station) %>%
        select(c(Latitude,Longitude)) %>% 
        slice(1)  # take first row (assuming all rows are equal...)
      
      
      #.... Forecast ----
      # NOTE time is UTC BEGIN
      # get df with forecast(0-24h) for specified dates
      forecast <- data.frame()
      # date_seq_forecast <- date_seq[date_seq > as.Date(now()-dhours(48))] #48=72-24
      date_seq_forecast <- date_seq[date_seq >= as.Date(now()-ddays(4))] #4days in order to calculate adj
      for(i in 1:length(date_seq_forecast)){
        fname <- paste0('/var/rprojects/DATA/ECMWF/CAMS/ENS_FORECAST_',date_seq_forecast[i],'_0-72h.nc')
        if(file.exists(fname)){
          forecast_i <- nc_to_df(fname,latlon)
          forecast <- bind_rows(forecast,forecast_i)
        }
      }
      rm(forecast_i)
      
      if("PM25" %in% names(forecast)){names(forecast)[names(forecast)=='PM25'] <- 'PM2.5'}
      
      
      # drop duplicated rows at 00:00 with different values (keeps last repeated row)
      # because i am binding analysis from different days (overlap of H24_DayD with H0_DayD+1)
      forecast <- forecast %>% group_by(date) %>% filter(!duplicated(date)) 
      
      # adjust time to match other datasets (forecast and analysis were in utc begin, others in utc+1 end)
      forecast$date <- forecast$date + dhours(2)
      
      ##.Combine EEA+ECMWF----
      # merge EEA observations + cams with ECMWF cams forecast and analysis separately
      obs_cams_forecast <- obs_cams %>% full_join(forecast, by='date', suffix=c('','_cams_forecast'))
    }
    ...
    if(get_forecast){
      obs_cams_forecast_adj <- calc_cams_adjusted(obs_cams_forecast,poll.list,cams_name="_cams_forecast")
      
      # join cams_adjusted+forecast_adjusted
      obs_cams_adj <- obs_cams_adj %>% full_join(obs_cams_forecast_adj)
      # obs_cams_adj <- obs_cams_adj %>% right_join(obs_cams_forecast_adj)
    }
    ...
    for(pol in poll.list){
      ...
      if(get_forecast){
        obs_cams_adj[[paste0(pol,"_gapFill_forecast")]] <- gap_fill(obs_cams_adj,pol,paste0(pol,"_cams_forecast_adj"))
      }
    }
    ...
    #rolling means of Forecast PMs
    if("PM10_gapFill_forecast" %in% names(obs_cams_adj)){
      obs_cams_adj <- rollingMean(obs_cams_adj,pollutant="PM10_gapFill_forecast",width=24,
                              align="right",new.name="PM10_gapFill_forecast",data.capture=75)
    }
    if("PM2.5_gapFill_forecast" %in% names(obs_cams_adj)){
      obs_cams_adj <- rollingMean(obs_cams_adj,pollutant="PM2.5_gapFill_forecast",width=24,
                                  align="right",new.name="PM2.5_gapFill_forecast",data.capture=75)
    }
    #...
    ...
    if(get_forecast){
      index_cams <- calc_AQI_dec(obs_cams_adj,poll.list,suffix="_gapFill_forecast")  #using forecast
    }else{
...
# tops_all <- left_join(top_obs,top_app,by=c("date","station"),suffix=c("_obs","_app"))
